...
protected:
  bool cost_computed;
  double cost;
  double edge_weight;   //边权重
  double node_weight;   //结点权重or结点执行时间
...
public:
  ...
  void OutputToFile(string root, string filename) const
  {

    ofstream out;
    out.precision(20);
    cout << "filename:" << root + "/" + filename << endl;
    out.open(root + "/" + filename);

    if (out.is_open())
    {
      for (vector<Cnode *>::iterator iter = nodes->begin(); iter != nodes->end(); iter++)
      {
        out << (*iter)->GetId() << " " << max((unsigned int)0, (*iter)->GetParentId() /*+1-offset_id*/) << " " << (*iter)->GetNW() << " " << (*iter)->GetMSW() << " " << (*iter)->GetEW() << endl;
      }
    }
    out.close();
    return;
...
protected:
  ...
  unsigned int remainProcs;

public:
  Procs()
  {
  }
  Procs(unsigned int num_processors, double minMem)
  {
    this->num_processors = num_processors;
    this->remainProcs = num_processors;
    this->powers = getRandom(1, 0.5, num_processors);
    this->mems = new double[num_processors];
    this->isUsed = new bool[num_processors];
  ...
  Procs(unsigned int num_processors, double minMem, int memory_constraint)
  {
    this->num_processors = num_processors;
    this->remainProcs = num_processors;
    this->powers = getRandom(1, 0.5, num_processors);
    this->mems = new double[num_processors];
    this->isUsed = new bool[num_processors];
    if (memory_constraint == 1) //有内存限制
    {
      this->mems = getRandom(minMem, 50, num_processors);
    }
    else
    {
      //设置处理器大小为执行任务树所需的最小内存，即没有处理器限制。
      for (int i = 0; i < num_processors; i++)
      {
        this->mems[i] = minMem;
      }
  ...
  bool EnoughMem(int pid, double memory_required)
  {
    bool f=!this->IsUsedForPro(pid)&&(memory_required <= this->getMem(pid));
    return f;
  }
  double *getMems()
  {
  ...
  bool IsUsedForPro(int pid)
  {
    return isUsed[pid];
  }
  void setUsed(int pid)
  {
    isUsed[pid] = true;
    this->remainProcs--;
  }
  bool IsRemain()
  {
    return (this->remainProcs > 0);
  }
  int getRemainProcs()
  {
    return this->remainProcs;
  }

  void PrintProcs()
  {
    cout << "num_pro:" << this->num_processors << endl;
    cout << "processors:" << endl;
    for (int i = 0; i < this->num_processors; i++)
    {
      cout << this->powers[i] << "     " << this->mems[i] << endl;
    }
    cout << endl;
  }
...
private:
  void quickSortForPM()
  {
    quickSort(this->powers, this->mems, 0, num_processors - 1);
  }
};
