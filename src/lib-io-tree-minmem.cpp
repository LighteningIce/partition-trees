...
#else
//[2011 paper ]         i:node   Mavail : available_memory                 Linit : L_init       Trinit  :  S_init          Mi : cut_value    Li : min_sub_cut      Tri: sub_schedule   Mipeak: Mpeak
...
// cout<< node->GetCost()<<"  "<<available_memory<<endl;
...
#endif
    Mpeak = node->GetCost();
    cut_value = numeric_limits<double>::infinity( );//Mi : cut_value  
#ifdef DEBUG_USING_MINMEM
    if(mycount<2*N){
...
#endif
    sub_schedule.push_back(node->GetId());//Tri: sub_schedule
    cut_value = 0;
  ...
  if(L_init != NULL){
    if(L_init->size() > 0){
      min_sub_cut.assign(L_init->begin(),L_init->end());//Li : min_sub_cut
  ...
  for (list<Cnode*>::iterator current_node=candidates->begin(); current_node!=candidates->end(); ++current_node){
    ...
    for (list<Cnode*>::iterator other_nodes=min_sub_cut.begin(); other_nodes!=min_sub_cut.end(); ++other_nodes){
      if((*other_nodes)->GetId() != (*current_node)->GetId()){
        (*current_node)->Mavail -= (*other_nodes)->GetEW();//计算出current_node最大的执行时间
      }
    }
...
#endif
{
  //M 执行以i为根结点的子树最小内存，包括
  double M = numeric_limits<double>::infinity();//inf
  //Mpeak 访问一个新结点所需要的最小内存
  ...
  struct rlimit lim;
  lim.rlim_cur = RLIM_INFINITY;//取得或设置的资源[软限制]的值
  lim.rlim_max = RLIM_INFINITY;//[硬限制]

  setrlimit(RLIMIT_STACK,&lim);// 修改task进程资源上限值,   
  ...
  while(Mpeak < numeric_limits<double>::infinity()){
    ...
    // cout<< "Required_memory before:  " <<  Mpeak <<"  rootid="<<root->GetId()<< endl; 
#ifdef DEBUG_USING_MINMEM
    explore(root, R equired_memory, &L, &Schedule,  M, L, Schedule, Mpeak, quiet,0,count,minmem_trace,tree->GetNodes()->size());
#else
    explore(root, Required_memory, &L, &Schedule,  M, L, Schedule, Mpeak, quiet,0,count);
    // cout<< "Required_memory  after:  " <<  Mpeak << endl; 
#endif
