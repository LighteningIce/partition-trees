# setwd("~/ChangjiangGou/MemCom/journal/code/result")#设置当前工作文件夹
setwd("/home/hesn/MemComJournal/result")
library(ggplot2)#加载第三方包  ggplot2 绘图
library(reshape2)#reshape2数据处理操作包
library(tikzDevice)# tikzDevice包可以把R图形数据转化成tikz命令. 这些命令可以直接包含进LaTeX文档, 从而编译出与文档样式一致的精致图形
rm(list=ls())#删除所有变量
...
#dcast(data,formula,value.var)  data 数据集，formula  形如x~y，x为行标签，y为列标签，value.var要处理的数据
