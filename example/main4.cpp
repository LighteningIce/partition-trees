...
int main(int argc, char **argv)
{
    ...
    memory_constraint = memory_constraint_options[memory_constraint];
    PartitionTree(treeobj1, num_processors, quiet, method, memory_constraint);
    number_subtrees = HowmanySubtrees(treeobj1, quiet);
    makespan = treeobj1->GetRoot()->GetMSCost(true, true);
    time = clock() - time;
   

    //std::cout << treename << " " << NPR << " " << CCR << " " << memory_constraint << " " << number_subtrees << " " << num_processors << " " << makespan << " " << stage1 << " " << stage2 << " " << stage3 << " " << std::endl;
    cout << makespan<<"   "<<endl;; //
     //cout<<"   "<<time<<endl;
